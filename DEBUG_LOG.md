...
C04/R04（英文供应商邮件的检索排序，共 3 分）：检索层已有别名归一（salmon ↔ 三文鱼poke），剩余差距在"断供/赔偿"等跨语言同义词的词面重叠。继续调 `DOC_PRIOR`、乘数或硬编码"断供=supply failure"这类映射，能拿到这 3 分，但那是把公开题库的答案焊进代码——隐藏题库换问法后大概率负收益。评测分数之外，这也是本题反复强调的原则：**不写死任何数字、答案和文档内容**。我选择保留机制的通用性。
