...
| `CHAT_BUDGET` | 单次问答内部编排预算秒数 | 150 | 环境变量（可选，`kbqa/config.py`） |

- 三个变量**全部非空**即进入 `live` 模式，任一为空即 `mock`；`/api/health` 的
  `llm_mode` 字段实时反映。
- 不做启动时 Key 格式校验，不调用查余额/列模型等接口（§7.2 要求）。
- 契约要求 `/api/chat` 在 180 秒内返回；服务把内部 LLM 编排预算设为 150 秒，为最终渲染、
  序列化和网络抖动预留约 30 秒。单次模型调用最多 120 秒，并取"120 秒与内部剩余预算
  较小者"（`kbqa/llm.py LLMClient.chat_with_retry`）。
...
| 180 秒响应上限 | 内部编排预算默认 150 秒；每次调用超时 = min(120s, 内部剩余预算)，预算耗尽返回 refusal，为响应组装与网络抖动预留约 30 秒（P11：最慢 120.03s < 180s） |
